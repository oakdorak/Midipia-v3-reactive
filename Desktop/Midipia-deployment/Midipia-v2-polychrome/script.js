...
// --- VERSION 2: Polychrome (Dynamic Mixing) ---
// Cada nota añade un color al lienzo y se mezclan.

let activeColors = [];

function addPolychrome(midiNote) {
    const hue = (midiNote % 12) * 30;
    activeColors.push(hue);
    if (activeColors.length > 5) activeColors.shift();

    const gradient = activeColors.map(h => `hsl(${h}, 70%, 50%)`).join(', ');
